...
export default function Cart() {
  ...
  return (
    <main className="min-h-screen px-4 sm:px-6 md:px-10 py-10 md:py-14 max-w-7xl mx-auto flex flex-col lg:flex-row gap-10">
      ...
      >
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#DB0030] mb-6 sm:mb-8 leading-snug">
          Shopping Cart{' '}
          <span className="block sm:inline text-sm sm:text-base md:text-lg text-[#DB0030]/70 font-normal ml-1">
        ...
        {products.length > 0 && (
          <>
            ...
            <div className="space-y-4">
              {products.map((product) => (
                ...
                >
                  ...
                  <p className="text-center font-semibold text-[#004D98]">
                    ${Number(product.offerPrice * product.quantity).toLocaleString()}
                  </p>
                  <div className="flex justify-center sm:justify-end">
                    <button
                      onClick={() => handleRemove(product.id)}
                      className="text-[#004D98] hover:text-red-600 transition cursor-pointer"
            ...
            <button
              type="button"
              className="inline-flex items-center gap-1 mt-8 text-[#004D98] font-semibold text-sm sm:text-base hover:underline cursor-pointer"
      ...
      <aside className="max-w-md w-full bg-[#f3f9f7] p-6 sm:p-8 rounded-lg border border-[#DB0030]/30 shadow-md top-20 self-start relative">
        <h2 className="text-xl sm:text-2xl font-bold text-[#004D98] mb-5">Order Summary</h2>
        <div>
          <p className="uppercase font-semibold text-sm text-[#004D98] mb-2">Delivery Address</p>
          <div className="relative flex justify-between items-start">
            ...
            <button
              ...
              className="text-[#004D98] hover:underline text-sm font-medium cursor-pointer ml-2"
              type="button"
            ...
            >
              {savedAddresses.length === 0 ? (
                ...
                savedAddresses.map((address, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center px-4 py-2 hover:bg-[#DB0030]/10 text-sm"
                  >
                    <p onClick={() => handleAddressSelect(address)} className="cursor-pointer flex-1">
              ...
              <p
                ...
                className="cursor-pointer px-4 py-2 hover:bg-[#DB0030]/20 text-indigo-600 font-medium text-center border-t border-gray-300"
              >
                Add new address
              </p>
            </div>
        ...
        <div className="mt-6">
          <label
            ...
            className="uppercase font-semibold text-sm text-[#004D98] mb-2 block"
          >
            Payment Method
          </label>
          <select
        ...
        <button
          className="w-full mt-6 py-3 rounded bg-[#DB0030] text-white font-semibold hover:bg-[#9b0026] transition text-base cursor-pointer"
          type="button"
